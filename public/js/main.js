...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // --- START: UPDATED SLIDESHOW INITIALIZATION ---
    ...
    // --- END: UPDATED SLIDESHOW INITIALIZATION ---
...
function renderHomePage(renderFunc) {
    // --- START: POPULAR PACKAGES LOGIC ---
    // 'Unlimited' package එක සොයාගෙන, එය මැදට வரும் ලෙස සකස් කිරීම
    const allPlans = Object.entries(appData.plans).map(([key, plan]) => ({ key, ...plan }));
    let unlimitedPlan = null;
    let otherPlans = [];

    // Unlimited plan එක සොයා ගැනීම
    const unlimitedIndex = allPlans.findIndex(plan => /unlimited/i.test(plan.name));
    if (unlimitedIndex > -1) {
        unlimitedPlan = allPlans.splice(unlimitedIndex, 1)[0];
    }
    ...
    let displayPlans = [];
    if (unlimitedPlan) {
        // Unlimited plan එක මැදට දමා, දෙපැත්තට අනෙක් plans දෙක යෙදීම
        displayPlans.push(otherPlans[0] || null);
        displayPlans.push(unlimitedPlan);
        displayPlans.push(otherPlans[1] || null);
    } else {
        // Unlimited plan එක නොමැති නම්, පළමු 3 පෙන්වීම
        displayPlans = allPlans.slice(0, 3);
    }

    const popularPlansHtml = displayPlans.filter(p => p).map(plan => {
        const isUnlimited = /unlimited/i.test(plan.name);
        const cardClasses = `card reveal card-glass p-5 rounded-xl text-center flex flex-col ${isUnlimited ? 'highlight-card' : ''}`;

        return `
        <div class="${cardClasses}">
            ${isUnlimited ? '<div class="popular-badge">Most Popular</div>' : ''}
            ...
            <ul class="space-y-2 text-gray-300 text-sm text-left my-4 flex-grow">
                ${plan.features.map(f => `<li><i class="fa-solid fa-check text-green-400 mr-2"></i>${f}</li>`).join("")}
            </ul>
            <a href="/plans" class="nav-link-internal mt-6 inline-block ai-button rounded-lg">View All Plans</a>
        </div>
        `;
    }).join("");
    // --- END: POPULAR PACKAGES LOGIC ---

    renderFunc(`
        <div class="page" id="page-home">

            <div class="swiper-container my-8 reveal">
                <div class="swiper-wrapper">
                    <div class="swiper-slide card-glass">
                        <div class="slide-content">
                            <h2>Unmatched Speed for Sri Lanka</h2>
                            <p>Experience the lowest latency and highest speeds with our servers optimized for all major Sri Lankan networks.</p>
                        </div>
                        <img src="/assets/image.jpg" class="slide-image" alt="High Speed Connection">
                    </div>
                    <div class="swiper-slide card-glass">
                         <div class="slide-content">
                            <h2>Ultimate Privacy & Security</h2>
                            <p>Keep your online activities private. Our advanced V2Ray encryption protects you from monitoring and threats.</p>
                        </div>
                        <img src="/assets/NEX.jpg" class="slide-image" alt="Cyber Security">
                    </div>
                    <div class="swiper-slide card-glass">
                        <div class="slide-content">
                            <h2>Seamless Multi-Device Access</h2>
                            <p>Use a single NexGuard account simultaneously on your phone, laptop, and tablet. Freedom on all your devices.</p>
                        </div>
                        <img src="/assets/image.jpg" class="slide-image" alt="Multi-device support">
                    </div>
                </div>
                <div class="swiper-pagination"></div>
            </div>
            <div id="popular-packages" class="my-20 reveal">
                <header class="text-center mb-12">
                    <h2 class="text-2xl font-bold text-white">Our Most Popular Packages</h2>
                    <p class="text-gray-400 mt-2">Get started with one of our top-rated plans.</p>
                </header>
              ...
              <div class="space-y-4 max-w-3xl mx-auto">
                  <details class="card-glass p-5 rounded-lg cursor-pointer">
                      ...
                      <p class="text-gray-300 text-sm mt-3 pt-3 border-t border-white/10">
                          V2Ray is a powerful and flexible networking tool used to secure your internet connection and bypass restrictions.
                      </p>
                  </details>
                  <details class="card-glass p-5 rounded-lg cursor-pointer">
                      <summary class="font-semibold text-white flex justify-between items-center">
                      ...
                      <p class="text-gray-300 text-sm mt-3 pt-3 border-t border-white/10">
                          Our service works on Android (v2rayNG), iOS (FoXray, Shadowrocket), and Windows (v2rayN).
                      </p>
                  </details>
              </div>
          </div>
