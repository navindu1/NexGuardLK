...
const fetchClientData = async (username) => {
    // 1. දැනටමත් ඉල්ලීමක් යවා ඇත්නම්, අලුත් එකක් නොයවා එයම භාවිතා කරන්න (Deduplication)
    ...
    const promise = apiFetch(`/api/check-usage/${username}`)
        ...
        .then(result => {
            if (result.success) {
                ...
                // දත්ත ලැබුණු විගස LocalStorage එකට දමන්න
...
export function renderProfilePage(renderFunc, params) {
        ...
        <div id="help-modal" class="help-modal-overlay">
            <div class="help-modal-content grease-glass p-6 space-y-4 w-full max-w-md">
                <div class="flex justify-between items-start">
                    <div><h2 class="text-xl font-bold text-white font-['Orbitron']">Help & Support</h2><button id="lang-toggle-btn" class="text-xs text-blue-300 hover:text-white underline mt-1">English / සිංහල</button></div>
                    <button id="help-modal-close" class="text-white/80 hover:text-white text-3xl">&times;</button>
                </div>
                <div class="lang-content lang-en"><div><h3 class="text-lg font-semibold text-blue-300 mb-2">How to find Username?</h3><p class="text-gray-100 text-sm">Your username is the name assigned to your V2ray config. It's visible in your V2ray app next to the server name.</p></div></div>
                <div class="lang-content lang-si hidden"><div><h3 class="text-lg font-semibold text-blue-300 mb-2">Username සොයාගන්නේ කෙසේද?</h3><p class="text-gray-100 text-sm">ඔබගේ username යනු V2ray config ගොනුවට ලබා දී ඇති නමයි. එය V2ray app එකේ server එක අසල දිස්වේ.</p></div></div>
                <div class="bg-black/20 border border-white/10 rounded-xl p-2"><img src="/assets/help.jpg" class="rounded-lg w-full h-auto opacity-95"></div>
            </div>
        </div>`;
    ...
    const pageStyles = `<style>
        ...
        .help-modal-overlay { opacity: 0; visibility: hidden; transition: opacity 0.3s, visibility 0.3s; background: rgba(0, 0, 0, 0.2); z-index: 9999; position: fixed; inset: 0; display: flex; align-items: center; justify-content: center; padding: 1rem; }
        .help-modal-overlay.visible { opacity: 1; visibility: visible; }
        .help-modal-content { opacity: 0; transform: scale(0.90); transition: opacity 0.3s, transform 0.3s; }
    ...
    const renderPlanSelector = (activePlans, activePlanIndex = 0) => {
        ...
        document.querySelector('#plan-menu .floating-menu')?.addEventListener('click', (e) => {
            ...
            if (link) {
                ...
                localStorage.setItem('nexguard_last_selected_plan', activePlans[index].v2rayUsername); // Save Selection
    ...
    const loadProfileData = async () => {
        try {
            ...
            try { cachedPlansStr = localStorage.getItem('nexguard_plans_cache'); } catch(e){}
            ...
            if(data.activePlans) {
                try { localStorage.setItem('nexguard_plans_cache', JSON.stringify(data.activePlans)); } catch(e){}
            }
