    ...
    apiFetch("/api/user/status")
        ...
        .then((data) => {
            ...
            if (data.status === "approved" && data.activePlans?.length > 0) {
                ...
                const displayUserData = (data, name, container) => {
                    // Reusing the same display logic as in usage page for consistency
                    // But simplified for profile context if needed
                    ...
                    const html = `
                        <div class="grid grid-cols-2 gap-4 text-sm mt-4">
                            <div class="bg-black/20 rounded-lg p-4"><div class="flex items-center text-gray-400"><i class="fa-solid fa-circle-down text-sky-400 mr-2"></i><span>Download</span></div><p class="text-xl font-bold text-white mt-1">${formatBytes(down)}</p></div>
                            <div class="bg-black/20 rounded-lg p-4"><div class="flex items-center text-gray-400"><i class="fa-solid fa-circle-up text-violet-400 mr-2"></i><span>Upload</span></div><p class="text-xl font-bold text-white mt-1">${formatBytes(up)}</p></div>
                            <div class="bg-black/20 rounded-lg p-4 col-span-2"><div class="flex items-center justify-between text-gray-400"><div class="flex items-center"><i class="fa-solid fa-database text-green-400 mr-2"></i><span>Data Usage</span></div><span class="text-white font-bold">${usagePercentage.toFixed(1)}%</span></div><div class="w-full bg-black/30 rounded-full h-2.5 mt-2"><div class="bg-gradient-to-r from-sky-500 to-blue-500 h-2.5 rounded-full" style="width: ${usagePercentage}%"></div></div><p class="text-right text-xs text-gray-500 mt-1">${formatBytes(totalUsed)} / ${formatBytes(totalQuota)}</p></div>
                        </div>`;
                ...
                const displayPlanDetails = (planIndex) => {
                    ...
                    if (isNaN(planIndex) || planIndex < 0 || planIndex >= data.activePlans.length) planIndex = 0;
                    ...
                    try {
                        ...
                        qrContainer.innerHTML = `<p class="text-xs text-red-500 text-center p-4">QR error.</p>`;
                    }
                    ...
                    const loadMyOrders = async () => {
                        ...
                        try {
                            ...
                            const ordersHtml = (orders.length > 0) ? orders.map(order => {
                                ...
                                const connName = appData.connections.find(c => c.name === order.conn_id)?.name || order.conn_id;
                                const planName = appData.plans[order.plan_id]?.name || order.plan_id;
                                return `<div class="card-glass p-4 rounded-lg flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4"><div><p class="font-bold text-white">${planName} <span class="text-gray-400 font-normal">for</span> ${connName}</p><p class="text-xs text-gray-400 mt-1">Ordered on: ${new Date(order.created_at).toLocaleDateString()} ${order.status === 'approved' && order.final_username ? `| V2Ray User: <strong class="text-blue-300">${order.final_username}</strong>` : ''}</p></div><div class="text-sm font-semibold capitalize flex items-center gap-2 ${statusColors[order.status] || 'text-gray-400'}"><i class="${statusIcons[order.status] || 'fa-solid fa-question-circle'}"></i><span>${order.status}</span></div></div>`;
                    ...
                    const switchTab = (tabId) => {
                        ...
                        setupEventListeners(); // Re-bind settings form listener if tab is settings
                    };
                    ...
                    floatingMenu.addEventListener('click', (e) => {
                        ...
                        if (link) {
                            ...
                            if (planIndex !== undefined) {
                                document.querySelector('#plan-menu .trigger-menu .text').textContent = data.activePlans[planIndex].v2rayUsername;
