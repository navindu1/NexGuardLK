...
// --- Prefix එක කපලා පිරිසිදු නම ගන්න Global Function එක ---
const getDisplayName = (uname) => (uname ? uname.replace(/^[A-Za-z0-9]+_/, "") : "");
// --------------------------------------------------------

export function renderPlansPage(renderFunc, params) {
  // Screen Freeze Fix
  document.body.style.pointerEvents = "auto";
  const stuckOverlays = document.querySelectorAll(".fixed.inset-0.bg-black\\/80");
  stuckOverlays.forEach((overlay) => overlay.remove());

  const userToChange = params.get("change");
  const displayUserToChange = getDisplayName(userToChange);
  const changeQuery = userToChange ? `&change=${encodeURIComponent(userToChange)}` : "";

  let plansHtml = Object.entries(appData.plans)
    .map(
      ([key, plan]) => `
        <div class="card reveal card-glass p-5 custom-radius text-center flex flex-col relative z-10">
            ...
            <a href="/connections?planId=${key}${changeQuery}" class="nav-link-internal mt-6 inline-block ai-button rounded-lg relative z-20">Select Plan</a>
        </div>`,
    )
    .join("");
  ...
  renderFunc(`
        <div id="page-plans" class="page">
            <header class="text-center mb-10 reveal relative z-10">
                <h2 class="text-2xl font-bold text-white">${displayUserToChange ? "Select Your New Plan" : "Our V2Ray Plans"}</h2>
                <p class="text-gray-400 mt-2">${displayUserToChange ? `You are changing the plan for ${displayUserToChange}` : "Step 1: Choose your desired data package."}</p>
            </header>
...
export function renderConnectionsPage(renderFunc, params) {
  ...
  if (!planId || !appData.plans[planId]) {
    renderFunc(
      '<div class="page text-center"><p class="text-red-400">Invalid plan selection.</p><a href="/plans" class="nav-link-internal underline mt-2">Go back to plans</a></div>',
    );
    return;
  }
    ...
    appData.connections.length > 0
      ? appData.connections
          .map((conn) => {
            ...
            // h-[160px] වෙනුවට h-full දැම්මා. එතකොට ලොකුම කාඩ් එකේ ගාණට අනිත් ඒවත් ඉබේම ඇදෙනවා!
            return `<a href="${linkUrl}" class="nav-link-internal card reveal selectable card-glass custom-radius text-center flex flex-col items-center justify-center w-full h-full" style="padding: 2.5rem 0">
          ...
          .join("")
      : '<div class="text-amber-400 text-center col-span-full"><p>No connection types are currently available.</p></div>';

  // Desktop එකේදී grid-cols-3 සහ Mobile එකේදී grid-cols-1 වෙනස් වෙන්න හැදුවා
  renderFunc(`
        <div id="page-connections" class="page">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 max-w-5xl mx-auto w-full px-4 sm:px-0">${connectionsHtml}</div>
...
export function renderPackageChoicePage(renderFunc, params) {
  ...
  renderFunc(`
        <div id="page-package-choice" class="page">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 max-w-5xl mx-auto w-full px-4 sm:px-0">${choiceHtml}</div>
...
export function renderCheckoutPage(renderFunc, params) {
  ...
  const displayUserToRenew = getDisplayName(userToRenew);
  const displayUserToChange = getDisplayName(userToChange);
  ...
  if (plan && (conn || isRenewal)) {
    ...
    let renewalInfo = "";
    if (isRenewal) {
      renewalInfo = `<p class="mt-2 text-center">You are renewing for V2Ray user: <strong class="text-blue-400">${displayUserToRenew}</strong>.</p>`;
    }

    let changeInfo = "";
    if (isChange) {
      changeInfo = `<p class="mt-2 text-center text-amber-400">You are changing the plan for: <strong class="text-white">${displayUserToChange}</strong>. The old plan will be deleted upon approval.</p>`;
  ...
  renderFunc(`
        <style>
          ...
          /* Auto-generate button inside input styling */
          .username-input-with-btn { padding-right: 2.8rem !important; }
        </style>
        <div id="page-checkout" class="page">
            <div class="w-full max-w-sm mx-auto card-glass custom-radius p-6 reveal">
                    ...
                    <form id="checkout-form" class="space-y-4">
                        ...
                        <div class="form-group ${isRenewal ? "pb-2" : ""}">
                            <div class="relative w-full">
                                <input type="text" id="checkout-username" name="username" class="form-input w-full ${!isRenewal ? "username-input-with-btn" : ""} ${isRenewal ? "renewal-username-field" : ""}" required placeholder=" " value="${isRenewal ? displayUserToRenew : isChange ? displayUserToChange : userSession.username || ""}" ${isRenewal ? "readonly" : ""} autocomplete="off">
                                <label class="form-label">${isChange ? "New V2Ray Username" : "V2Ray Username"}</label><span class="focus-border"><i></i></span>
                                
                                ${
                                  !isRenewal
                                    ? `
                                <button type="button" id="btn-generate-user" class="absolute right-2 top-1/2 -translate-y-1/2 bg-white/5 hover:bg-white/15 text-blue-400 rounded-full w-8 h-8 flex items-center justify-center transition-all backdrop-blur-sm" title="Auto Generate Username">
                            ...
                            <div class="min-h-[20px] mt-2">
                                ${!isRenewal ? `<div id="username-status" class="text-xs px-1 text-left text-gray-400">Please enter a unique username.</div>` : ""}
                                ${isRenewal ? '<p class="text-xs text-amber-400 px-1 text-left">Username cannot be changed during renewal.</p>' : ""}
                            </div>
                        </div>
                ...
                <div id="success-view" class="hidden text-center">
                    ...
                    <p class="text-gray-300 mt-6 text-sm">Join our WhatsApp group for the latest updates, support, and special offers!</p>
  ...
  if (!isRenewal) {
    ...
    const generateBtn = document.getElementById("btn-generate-user");
    const submitBtn = document.getElementById("checkout-submit-btn");
    let debounceTimer;

    const checkUsername = async (username) => {
      if (!username || username.length < 3) {
        statusText.innerHTML = `<span class="text-gray-400"><i class="fa-solid fa-circle-info mr-1"></i> Enter at least 3 characters.</span>`;
        submitBtn.disabled = true;
        return;
      }

      if (displayUserToChange && username.trim().toLowerCase() === displayUserToChange.toLowerCase()) {
        statusText.innerHTML = `<span class="text-green-400"><i class="fa-solid fa-check-double mr-1"></i> You are keeping your old username.</span>`;
        submitBtn.disabled = false;
        return;
      }

      statusText.innerHTML = `<span class="text-blue-400"><i class="fa-solid fa-spinner fa-spin mr-1"></i> Checking availability...</span>`;
      submitBtn.disabled = true;
      ...
      try {
        ...
        if (res.ok) {
          if (data.available === true) {
            ...
            statusText.innerHTML = `<span class="text-red-400"><i class="fa-solid fa-xmark mr-1"></i> Already taken. Try: <b class="cursor-pointer text-amber-400 underline hover:text-amber-300 transition-colors" id="suggested-username">${data.suggestion}</b></span>`;
            document.getElementById("suggested-username")?.addEventListener("click", () => {
              usernameInput.value = data.suggestion;
              checkUsername(data.suggestion);
            });
          }
        } else {
        ...
        statusText.innerHTML = `<span class="text-amber-400"><i class="fa-solid fa-triangle-exclamation mr-1"></i> Network Error.</span>`;
        submitBtn.disabled = false;
      }
    };
    ...
    usernameInput.addEventListener("input", (e) => {
      ...
      debounceTimer = setTimeout(() => checkUsername(e.target.value.trim()), 600);
    });

    generateBtn.addEventListener("click", () => {
      const prefix = userSession.username ? userSession.username.substring(0, 4) : "nex";
      const randomStr = Math.random().toString(36).substring(2, 6);
      const generated = `${prefix}_${randomStr}`;
      usernameInput.value = generated;
      checkUsername(generated);

      generateBtn.style.transform = "scale(0.9) translateY(-50%)";
      setTimeout(() => (generateBtn.style.transform = "scale(1) translateY(-50%)"), 150);
    });
  ...
  document.getElementById("checkout-form")?.addEventListener("submit", async (e) => {
    ...
    if (renewUser) {
      ...
      if (!formData.get("username")) formData.append("username", getDisplayName(renewUser)); 
    }
    ...
    try {
      const res = await apiFetch("/api/create-order", { method: "POST", body: formData });
      if (res.ok) {
...
export function renderRenewOrChangePage(renderFunc, planToManage) {
  ...
  const displayUsername = getDisplayName(planToManage.v2rayUsername);

  renderFunc(`
        <div id="page-renew-choice" class="page">
            <header class="text-center mb-10 reveal">
                <h2 class="text-2xl font-bold text-white">Manage Plan: <span class="gradient-text">${displayUsername}</span></h2>
  ...
  return new Promise((resolve) => {
    ...
    const options = plansToShow
      .map((p, index) => {
        ...
        const displayUsername = getDisplayName(p.v2rayUsername);
        return `<option value="${index}">${displayUsername}${statusText}</option>`;
      })
      .join("");
