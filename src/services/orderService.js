...
exports.approveOrder = async (orderId, isAutoConfirm = false) => {
  ...
  try {
    ...
    if (!inboundId || !vlessTemplate) {
      return {
        success: false,
        message: `Inbound ID or VLESS Template is missing for this order. Cannot approve.`,
      };
    }
    ...
    if (userFetchError || !websiteUser) {
      ...
      console.error(
        `Website user fetch error for order ${orderId}:`,
        userFetchError
      );
      return {
        success: false,
        message: `Website user "${order.website_username}" associated with this order was not found.`,
      };
    }
    ...
    if (planError || !planDetails) {
      return {
        success: false,
        message: `Plan details for "${order.plan_id}" not found in the database.`,
      };
    }
    ...
    if (order.status === "pending") {
      ...
      console.log(
        `[Order Processing] Status is 'pending' for order ${orderId}. Proceeding with V2Ray operations.`
      );

      if (order.is_renewal) {
        // --- Renewal Logic ---
        const clientInPanel = await v2rayService.findV2rayClient(
          order.username
        );
        if (!clientInPanel) {
          return {
            success: false,
            message: `Renewal failed: User '${order.username}' not found in the panel.`,
          };
        }
        ...
        if (currentExpiryTime > now) {
          ...
          console.log(
            `[Queued Renewal] Plan for ${order.username} (Order: ${orderId}) is still active. Queuing renewal.`
          );
          const planDetailsForQueue = {
            /* ... (details as before) ... */
          }; // Fill details as in original code
          const { error: queueError } = await supabase
            .from("renewal_queue")
            .insert({
              /* ... (queue details) ... */
            }); // Insert details as in original code
          ...
          await supabase
            ...
            .update({
              ...
              final_username: order.username,
            })
            .eq("id", orderId);
          return {
            success: true,
            message: `Plan is still active. Renewal for ${order.username} has been queued successfully.`,
          };
        } else {
          // --- CONDITION B: PLAN HAS EXPIRED (Renew Immediately) ---
          console.log(
            `[Immediate Renewal] Plan for ${order.username} (Order: ${orderId}) has expired. Renewing immediately.`
          );
          ...
          await v2rayService.updateClient(
            inboundId,
            clientInPanel.client.id,
            updatedClientSettings
          );
          await v2rayService.resetClientTraffic(
            inboundId,
            clientInPanel.client.email
          );

          clientLink = v2rayService.generateV2rayConfigLink(
            vlessTemplate,
        ...
        if (allPanelClients.has(finalUsername.toLowerCase())) {
          ...
          console.log(
            `[Username Conflict] Generated new unique username for order ${orderId}: ${finalUsername}`
          );
        }
        ...
        console.log(
          `[DEBUG - New User] Order: ${orderId}, Plan: ${order.plan_id}, GB from DB: ${totalGBfromDB}, Final byte value: ${totalGBValue}`
        );
        ...
        const addClientResult = await v2rayService.addClient(
          inboundId,
          clientSettings
        );

        // Inside the 'else' block for NEW USER creation, AFTER v2rayService.addClient

        // Check if adding the client was successful before attempting to update
        if (addClientResult && addClientResult.success) {
          console.log(
            `[Quota Update] Attempting to explicitly update quota for new user: ${finalUsername}`
          );
          try {
            // Use the same clientSettings which contains the correct 'total' byte value
            // The client UUID is clientSettings.id
            const updateResult = await v2rayService.updateClient(
              inboundId,
              clientSettings.id,
              clientSettings
            ); // Use clientSettings.id as UUID
            if (updateResult && updateResult.success) {
              console.log(
                `[Quota Update] Successfully updated quota for ${finalUsername}.`
              );
            } else {
              // Log a warning if update fails, but don't stop the whole process
              console.warn(
                `[Quota Update] FAILED to update quota for ${finalUsername}. Panel Response: ${
                  updateResult?.msg || "No message"
                }. User might have unlimited data initially.`
              );
            }
          } catch (updateError) {
            console.warn(
              `[Quota Update] EXCEPTION during quota update for ${finalUsername}: ${updateError.message}. User might have unlimited data initially.`
            );
          }
        } else {
          // If addClient failed, we shouldn't proceed. The error would have been thrown already.
          console.warn(
            `[Quota Update] Skipping quota update for ${finalUsername} because initial addClient failed or response was unexpected.`
          );
        }

        // The rest of the code (createdV2rayClient = ..., clientLink = ...) remains the same after this new block
      ...
      console.log(
        `[Admin Confirm] Order ${orderId} is 'unconfirmed'. Skipping V2Ray creation/update, proceeding to finalize.`
      );
      ...
      if (!finalUsername) {
        ...
        throw new Error(
          `Critical: final_username is missing for unconfirmed order ${orderId}. Cannot proceed with approval.`
        );
      }
      ...
      if (!clientInPanel) {
        ...
        throw new Error(
          `Critical: V2Ray client ${finalUsername} (expected for unconfirmed order ${orderId}) not found in panel.`
        );
      }
      clientLink = v2rayService.generateV2rayConfigLink(
        vlessTemplate,
      ...
      if (!clientLink) {
        ...
        throw new Error(
          `Could not regenerate client link for ${finalUsername} (Order: ${orderId}). Template might be invalid.`
        );
      }
      console.log(
        `[Admin Confirm] Successfully retrieved existing client details and link for ${finalUsername}.`
      );
    } else {
      // Should not happen if the initial status check is correct, but added as a safeguard
      console.error(
        `[Order Processing Error] Order ${orderId} has an unexpected status: ${order.status}. Aborting.`
      );
      return {
        success: false,
        message: `Order has an unexpected status: ${order.status}`,
      };
    }
    // --- End Change 2 ---
    ...
    if (finalUsername && clientLink) {
      ...
      if (order.is_renewal) {
        planIndex = updatedActivePlans.findIndex(
        ...
        planIndex = updatedActivePlans.findIndex(
          (p) =>
            p.v2rayUsername.toLowerCase() ===
            order.old_v2ray_username.toLowerCase()
        );
      ...
      if (userUpdateError) {
        console.error(
          `Failed to update active_plans for user ${websiteUser.username} (Order: ${orderId}):`,
          userUpdateError
        );
      ...
      console.warn(
        `[Order Finalization Warning] finalUsername or clientLink is missing for order ${orderId}. Skipping user active_plans update.`
      );
    }
    ...
    if (orderUpdateError) {
      throw new Error(
        `Failed to update order status for ${orderId}: ${orderUpdateError.message}`
      );
    }

    // Send approval email ONLY if manually approved (finalStatus is 'approved')
    if (finalStatus === "approved" && websiteUser.email) {
      const emailSubject = `Your NexGuard Plan is ${
        order.is_renewal ? "Renewed" : "Activated"
      }!`;
      ...
      try {
        ...
        console.log(
          `[Email Sent] Approval email sent successfully to ${websiteUser.email} for order ${orderId}.`
        );
      } catch (emailError) {
        // Log the failure but don't stop the overall success response
        console.error(
          `[Email Error] FAILED to send approval email for order ${orderId} to ${websiteUser.email}:`,
          emailError
        );
      }
    }
    ...
    if (order.old_v2ray_username && finalStatus === "approved") {
      // Only cleanup on final approval
      try {
        const oldClientData = await v2rayService.findV2rayClient(
          order.old_v2ray_username
        );
        if (oldClientData) {
          await v2rayService.deleteClient(
            oldClientData.inboundId,
            oldClientData.client.id
          );
          console.log(
            `[Plan Change Cleanup] Successfully deleted old user: ${order.old_v2ray_username} after approving order ${orderId}.`
          );
        }
      } catch (cleanupError) {
        // Log critical error for manual intervention
        console.error(
          `[CRITICAL - Cleanup Failed] Failed to delete old V2Ray user ${order.old_v2ray_username} for order ${orderId}. Please delete manually. Error: ${cleanupError.message}`
        );
      }
    }
    ...
    return {
      ...
      message: `Order for ${
        finalUsername || order.username
      } successfully set to ${finalStatus}.`,
      finalUsername,
    };
  } catch (error) {
    // --- Error Handling & Rollback ---
    console.error(
      `[CRITICAL ERROR] Error processing order ${orderId}:`,
      error.message,
      error.stack
    );
    ...
    if (createdV2rayClient) {
      console.log(
        `[ROLLBACK] An error occurred processing order ${orderId}. Attempting to delete newly created V2Ray client ${createdV2rayClient.settings.email}...`
      );
      try {
        await v2rayService.deleteClient(
          createdV2rayClient.inboundId,
          createdV2rayClient.settings.id
        );
        console.log(
          `[ROLLBACK SUCCESS] Successfully deleted V2Ray client for order ${orderId}.`
        );
      } catch (rollbackError) {
        // Log critical error for manual intervention
        console.error(
          `[CRITICAL - ROLLBACK FAILED] FAILED TO ROLLBACK V2Ray client ${createdV2rayClient.settings.email} for order ${orderId}. PLEASE DELETE MANUALLY. Error: ${rollbackError.message}`
        );
      }
    }
    ...
    return {
      success: false,
      message:
        error.message || "An unexpected error occurred during order approval.",
    };
  }
};
...
exports.processAutoConfirmableOrders = async () => {
  ...
  try {
    ...
    const enabledSettings = settings
      .filter(
        (s) =>
          s.key.startsWith("auto_approve_") &&
    ...
    if (ordersToProcess && ordersToProcess.length > 0) {
      console.log(
        `[Auto-Confirm] Found ${ordersToProcess.length} order(s) to process.`
      );
      ...
      for (const order of ordersToProcess) {
        ...
        if (approvalResult.success) {
          console.log(
            `[Auto-Confirm] Order ${order.id} processed successfully. Message: ${approvalResult.message}`
          );
        } else {
          console.error(
            `[Auto-Confirm] FAILED to process order ${order.id}. Reason: ${approvalResult.message}`
          );
        }
      }
    }
